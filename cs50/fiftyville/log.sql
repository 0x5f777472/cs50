...
-- this (below) gives names of the people
SELECT name FROM people WHERE phone_number IN (SELECT caller FROM phone_calls WHERE caller = '(367) 555-5533' AND day=28 AND duration<60);
SELECT name FROM people WHERE phone_number IN (SELECT receiver FROM phone_calls WHERE caller = '(367) 555-5533' AND day=28 AND duration<60);
-- this below gives destination of his flight
SELECT city FROM airports WHERE id IN (SELECT destination_airport_id FROM flights WHERE id IN (SELECT flight_id FROM passengers WHERE passport_number IN (SELECT passport_number FROM people WHERE phone_number = '(367) 555-5533')));